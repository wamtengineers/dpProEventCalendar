jQuery(function($) {
	...
	function onEventCategoryChange(event) {
		var $el = $(this),
			$parent = $el.closest('#pec_events_categorychecklist'),
			$inputs = [],
			blnChecked = this.checked,
			sId = this.id;
		...
		if ($parent.length) {
			...
			if (blnChecked) {
				$inputs.each(function() {
					if (this.checked === true && this.id !== sId) {
						this.checked = false;
					}
				});
			}
	...
	function setCategoryEvents() {
		$('input[id*=in-pec_events_category]').on('change', onEventCategoryChange);
	}
...
function pec_changeCurrentBooking(booking_id, event) {
	event.preventDefault();
	event.stopPropagation();

	var $button = $(event.target),
		$parent = $button.closest('td'),
		$select = $parent.find('select').first();

	if($select.length) {
		$.post(ProEventCalendarAjax.ajaxurl, {
			booking_id: booking_id,
			status: $select.val(),
			action: 'changeSingleStatus',
			postEventsNonce : ProEventCalendarAjax.postEventsNonce
		},
		function(data) {
			data = JSON.parse(data);
			if(data.result) {
				console.info('Update single status [OK] of booking id ' + data.booking_id);
			}
		});
	}
}

function pec_changeAllStatusBookins(post_id, event) {
	event.preventDefault();
	event.stopPropagation();

	$.post(ProEventCalendarAjax.ajaxurl, {
		eventid: post_id,
		status: $('#all_pec_booking_status').val(),
		action: 'changeAllStatusBookins',
		postEventsNonce : ProEventCalendarAjax.postEventsNonce
	},
	function(data) {
		data = JSON.parse(data);
		if(data.result) {
			var $selects = $('table.pec_booking_table > tbody > tr > td > select');
			if($selects.length) {
				$selects.each(function() {
					...
					$element.find('option').each(function() {
						var $option = $(this),
							sValue = $option.attr('value');

						if(sValue == data.status) {
