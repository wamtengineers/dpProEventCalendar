		...
		$(document).on('click', '.pec_edit_event', function(e) {
			...
			$.post(ProEventCalendarAjax.ajaxurl,
				...
				function(data) {
					...
					$(el).on('click', '.dp_pec_cancel_booking', function(e) {
						e.preventDefault();
						e.stopPropagation();
						var $button = $(this),
							$form = $button.closest('form'),
							$status = $form.find('#pec_custom_aux_estatus'),
							blnChecked = false;

						$button.prop('disabled', true);
						$button.addClass('dp_pec_disabled');
						if ($status.length) {
							blnChecked = $status.get(0).checked;
						}

						if (!blnChecked) {
							var blnConfirm = confirm('¿Está seguro de cerrar el evento? Una vez cerrado no se podrá editar. Para cerrar el evento debe estar completa la lista de participantes y ajustado el kilometraje final. Para ajustar estos datos presione Regresar o Confirmar para cerrar el evento.');

							if (blnConfirm) {
								$.post(ProEventCalendarAjax.ajaxurl, {
									eventid: $form.find('input[name="edit_event"]').val(),
									kilometraje: $form.find('input#pec_custom_kilometraje').val(),
									action: 'endSingleEvent',
									postEventsNonce : ProEventCalendarAjax.postEventsNonce
								},
								function(data) {
								...
								$button.removeClass('dp_pec_disabled');
							}
						}
					});
					...
					$('#pec_new_booking_submit').click(function(event) {
						var sSelectedUser = $('#pec_new_booking_user').val(),
							$button = $(this);

						$('#pec_new_booking_user').prop('disabled', true);
						$button.prop('disabled', true);
						$.post(ProEventCalendarAjax.ajaxurl, {
								...
								try {
									...
									if(oJson.html) {
										if($('#pec_booking_list_zero').length) {
											$('#pec-booking-list').html(oJson.html);
										} else {
											$('#pec-booking-list').prepend(oJson.html);
										}
									...
									$('#pec_new_booking_user').find('option[value="' + sSelectedUser + '"]').remove();
									$('#pec_new_booking_user').prop('disabled', false);
									$button.prop('disabled', false);
								} catch(err) {
									console.error(err);
								}
					...
					// 	increaseArea: '20%' // optional
					// });
					// $('.dpProEventCalendarModalEditEvent input.checkbox').on('ifChanged', function(event) {
					// 	Woffice.onEventCategoryChange.call(event.target);
					// });
				}
			);
			return false;
		...
		$(document).on('click', '.pec_event_page_send_booking', function(e) {
			...
			if($('#pec_event_page_book_name', '.dpProEventCalendarModal').length) {
				...
				if($('#pec_event_page_book_name', '.dpProEventCalendarModal').val() == '') {
					...
					sMessage = $('#pec_event_page_book_name', '.dpProEventCalendarModal').attr('placeholder') + " is required";
					blnHaveError = true;
				}
				...
				if($('#pec_event_page_book_email', '.dpProEventCalendarModal').val() == '' || !re.test($('#pec_event_page_book_email', '.dpProEventCalendarModal').val())) {
					...
					sMessage = $('#pec_event_page_book_email', '.dpProEventCalendarModal').attr('placeholder') + " is required";
					blnHaveError = true;
				}
			...
			if($('#pec_event_page_book_phone', '.dpProEventCalendarModal').length) {
				...
				if($('#pec_event_page_book_phone', '.dpProEventCalendarModal').val() == '') {
					...
					sMessage = $('#pec_event_page_book_phone', '.dpProEventCalendarModal').attr('placeholder') + " is required";
					blnHaveError = true;
				}
			...
			if(!is_valid) {
				sMessage = "The fields are required";
				blnHaveError = true;
			}
			...
			if($('#pec_event_page_book_terms_conditions', '.dpProEventCalendarModal').length) {
				...
				if($('#pec_event_page_book_terms_conditions', '.dpProEventCalendarModal').is( ":checked" ) == false) {
					...
					sMessage = "Terms and conditions must be accepted.";
					blnHaveError = true;
				}
			...
			if(blnHaveError) {
				var sModalText = [
						'<div id="custom-dpevent-errors" class="dpProEventCalendarModal" style="display: none; z-index: 999999992">',
							'<div class="pec_book_select_date">',
								'<p>',
										sMessage,
								'</p>',
								'<div class="pec-add-footer">',
									'<button class="pec_action_btn" type="button">Ok</button>',
								'</div>',
								'<div class="dp_pec_clear"></div>',
							'</div>',
						'</div>',
						'<div id="custom-dpevent-overlay" class="dpProEventCalendarOverlay" style="z-index: 999999991"></div>',
					].join(''),
					$modal = $('#custom-dpevent-errors');

				if($modal.length == 0) {
					$modal = $(sModalText);
					$modal.find('button').on('click', function() {
						var $button = $(this),
							$parent = $button.closest('.dpProEventCalendarModal');

						if($parent.length) {
							$parent.fadeOut('fast');
							$parent.next('.dpProEventCalendarOverlay').css('display', 'none');
						}
					})
					$('body').append($modal);
				}
				else {
					$modal.find('p').text(sMessage);
				}

				$modal.fadeIn('fast');
				$modal.next('.dpProEventCalendarOverlay').css('display', 'block');

				return false;
			}
			else {
				$('#custom-dpevent-errors, #custom-dpevent-overlay').remove();
			}
