...
if ( !class_exists('PluginUpdateChecker_2_2_custom', false) ):
 ...
 * @access public
 */
class PluginUpdateChecker_2_2_custom {
	...
	public function requestInfo($queryArgs = array()){
		...
		if ( !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
			$pluginInfo = PluginInfo_2_2_custom::fromJson($result['body'], $this->debugMode);
			if (!isset($pluginInfo)) $pluginInfo = new stdClass();
	 ...
	 *
	 * @uses PluginUpdateCheckerCustom::requestInfo()
	 *
	 * @return PluginUpdateCustom An instance of PluginUpdateCustom, or NULL when no updates are available.
	 */
	public function requestUpdate(){
		...
		return PluginUpdate_2_2_custom::fromPluginInfo($pluginInfo);
	}
	 ...
	 *
	 * @return PluginUpdateCustom|null
	 */
	...
	public function getUpdateState() {
		...
		if ( !empty($state) && isset($state->update) && is_object($state->update) ){
			$state->update = PluginUpdate_2_2_custom::fromObject($state->update);
		}
		return $state;
	}
	...
	private function setUpdateState($state) {
		if ( isset($state->update) && is_object($state->update) && method_exists($state->update, 'toStdClass') ) {
			$update = $state->update; /** @var PluginUpdateCustom $update */
	 ...
	 * successfully, the first argument passed will be an instance of  PluginInfoCustom. Otherwise, 
	 * it will be NULL. The second argument will be the corresponding return value of 
	 * wp_remote_get (see WP docs for details).
	 *  
	 * The callback function should return a new or modified instance of PluginInfoCustom or NULL.
	 * 
	 * @uses add_filter() This method is a convenience wrapper for add_filter().
	 * 
	 * @param callable $callback
...
if ( !class_exists('PluginInfo_2_2_custom', false) ):
...
class PluginInfo_2_2_custom {
	...
	/**
	 * Create a new instance of PluginInfoCustom from JSON-encoded plugin info 
	 * returned by an external update API.
	 * 
	 * @param string $json Valid JSON string representing plugin info.
	 * @param bool $triggerErrors
	 * @return PluginInfoCustom|null New instance of PluginInfoCustom, or NULL on error.
	 */
...
if ( !class_exists('PluginUpdate_2_2_custom', false) ):
...
class PluginUpdate_2_2_custom {
	...
	/**
	 * Create a new instance of PluginUpdateCustom from its JSON-encoded representation.
	 * 
	 * @param string $json
	 * @param bool $triggerErrors
	 * @return PluginUpdateCustom|null
	...
	public static function fromJson($json, $triggerErrors = false){
		...
		$pluginInfo = PluginInfo_2_2_custom::fromJson($json, $triggerErrors);
		if ( $pluginInfo != null ) {
	...
	/**
	 * Create a new instance of PluginUpdateCustom based on an instance of PluginInfoCustom.
	 * Basically, this just copies a subset of fields from one object to another.
	 * 
	 * @param PluginInfoCustom $info
	 * @return PluginUpdateCustom
	 */
	...
	/**
	 * Create a new instance of PluginUpdateCustom by copying the necessary fields from 
	 * another object.
	 *  
	 * @param StdClass|PluginInfoCustom|PluginUpdateCustom $object The source object.
	 * @return PluginUpdateCustom The new copy.
	 */
	 ...
	 * a PluginUpdateCustom. Useful for serialization and caching, as it avoids
	...
	public function toWpFormat(){
		...
		if($dpProEventCalendar['purchase_code'] == "") {
			$dpProEventCalendar['purchase_code'] = esc_attr( get_site_option('pec-purchase-code'));
		}
		...
		$update->package = $this->download_url.'?purchaseCode='.$dpProEventCalendar['purchase_code'].'&domain='.str_replace(".", "-", urlencode(site_url()));
...
if ( !class_exists('PucFactoryCustom', false) ):

/**
 * A factory that builds instances of other classes from this library.
 *
 * When multiple versions of the same class have been loaded (e.g. PluginUpdateCheckerCustom 1.2
 * and 1.3), this factory will always use the latest available version. Register class
 * versions by calling {@link PucFactoryCustom::addVersion()}.
 *
 * At the moment it can only build instances of the PluginUpdateCheckerCustom class. Other classes
 * are intended mainly for internal use and refer directly to specific implementations. If you
 * want to instantiate one of them anyway, you can use {@link PucFactoryCustom::getLatestClassVersion()}
 * to get the class name and then create it with <code>new $class(...)</code>.
 */
class PucFactoryCustom {
	protected static $classVersions = array();
	protected static $sorted = false;

	/**
	 * Create a new instance of PluginUpdateCheckerCustom.
	 *
	 * @see PluginUpdateCheckerCustom::__construct()
	 *
	 * @param $metadataUrl
	 * @param $pluginFile
	 * @param string $slug
	 ...
	 *
	 * @param string $generalClass Class name without version numbers, e.g. 'PluginUpdateCheckerCustom'.
...
PucFactoryCustom::addVersion('PluginUpdateCheckerCustom', 'PluginUpdateChecker_2_2_custom', '2.2');
PucFactoryCustom::addVersion('PluginUpdateCustom', 'PluginUpdate_2_2_custom', '2.2');
PucFactoryCustom::addVersion('PluginInfoCustom', 'PluginInfo_2_2_custom', '2.2');
PucFactoryCustom::addVersion('PucGitHubCheckerCustom', 'PucGitHubChecker_2_2_custom', '2.2');
 ...
 */
if ( !class_exists('PluginUpdateCheckerCustom', false) ) {
	class PluginUpdateCheckerCustom extends PluginUpdateChecker_2_2_custom { }
}

if ( !class_exists('PluginUpdateCustom', false) ) {
	class PluginUpdateCustom extends PluginUpdate_2_2_custom {}
}

if ( !class_exists('PluginInfoCustom', false) ) {
	class PluginInfoCustom extends PluginInfo_2_2_custom {}
}
