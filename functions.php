...
// Código personalizado para adaptar el PLUGIN.
/**
 * Obtiene la información de un evento, el cual viene siendo un POST
 * @param {int} $id_event
 */
function get_event_info_custom($id_event) {
...
/**
 * Se encarga de terminar de procesar el evento cuando se
 * elimina un usuario de un evento.
 * @param  {int}    $distance   Distancia
 * @param  {string} $end_date   Fecha fin del evento
 */
function proccess_on_action_booking($eventid, $distance, $id_user, $bln_sum) {
	// Obtenemos la lista de los eventos a los que ha asistido
	$continue = true;
	$travels_list = xprofile_get_field_data(170, $id_user);
	$list = array();
	if(isset($travels_list)) {
		$list = explode(",", $travels_list);
		$key = array_search($eventid, $list);

		// Si es una suma lo que se va a relizar, y además ya existe el evento,
		// se cambia la bandera continue a false.
		if($bln_sum) {
			if($key > 0) {
				$continue = false;
			}
		}
		// En otro caso, si no es una suma, comprobamos que exista el evento
		// sino existe, no continuamos.
		else if($key === false) {
			$continue = false;
		}
	}
	...
	if($continue) {
		// Obtenemos el numero de viajes que tiene el usuario
		$travels = xprofile_get_field_data(46, $id_user);

		// Le restamos un viaje y se lo cambiamos al usuario
		$int_traverls = intval($travels);
		if($bln_sum) {
			$int_traverls++;
		}
		else {
		...
		if($int_traverls > 0) {
			xprofile_set_field_data(46, $id_user, $int_traverls);
		}
		else {
			xprofile_set_field_data(46, $id_user, "0");
		}

		// Obtenemos los kilometros acumulados
		$km = xprofile_get_field_data(47, $id_user);
		...
		if($int_km > 0) {
			xprofile_set_field_data(47, $id_user, $int_km);
		}
		else {
			xprofile_set_field_data(47, $id_user, "0");
		}

		if($bln_sum) {
			array_push($list, $eventid);
		}
		else if(is_numeric($key)) {
			unset($list[$key]);
		}

		xprofile_set_field_data(170, $id_user, implode(",", $list));
	}
}

function on_remove_book_event_change($booking_id) {
	global $wpdb, $table_prefix;
	$table_name_booking = $table_prefix.DP_PRO_EVENT_CALENDAR_TABLE_BOOKING;

	$querystr = "SELECT * FROM $table_name_booking WHERE id = $booking_id";
	$booking = $wpdb->get_row($querystr, OBJECT);

	if(is_numeric($booking->id_user) && $booking->id_user > 0) {
		if(is_numeric($booking->id_event)) {
			$event = get_event_info_custom($booking->id_event);
			if(isset($event)) {
				// Obtenemos los kilometros del evento
				$distance = get_post_meta($event->ID, 'pec_custom_kilometraje', true);

				proccess_on_action_booking($booking->id_event, $distance, $booking->id_user, false);
			}
		}
	}
}

add_action( 'wp_ajax_nopriv_changeSingleStatus', 'dpProEventCalendar_changeSingleStatus' );
add_action( 'wp_ajax_changeSingleStatus', 'dpProEventCalendar_changeSingleStatus' );

function dpProEventCalendar_changeSingleStatus() {
	header("HTTP/1.1 200 OK");
	global $wpdb, $table_prefix;
	// Inicializamos el resultado en falso
	$response = array("result" => false);

	// Comprobamos que el usuario este logueado
	if(is_user_logged_in()) {
		// Comprobamos que exista y sea numerico el id del booking
		if(is_numeric($_POST['booking_id'])) {
			...
			// Obtenemos el nombre correspondiente a los books del plugin
			$table_booking = $table_prefix.DP_PRO_EVENT_CALENDAR_TABLE_BOOKING;

			$querystr = "UPDATE ".$table_booking." SET status = '".$status."' WHERE id = %d";
			$wpdb->query($wpdb->prepare($querystr, $booking_id));
...
add_action( 'wp_ajax_nopriv_endSingleEvent', 'dpProEventCalendar_endSingleEvent' );
add_action( 'wp_ajax_endSingleEvent', 'dpProEventCalendar_endSingleEvent' );

function dpProEventCalendar_endSingleEvent() {
	header("HTTP/1.1 200 OK");
	global $wpdb, $table_prefix;
	// Inicializamos el resultado en falso
	$response = array("result" => false);
	...
	if(is_user_logged_in()) {
		...
		if(is_numeric($_POST['eventid'])) {
			...
			// Obtenemos los kilometros del evento
			$distance = intval($_POST['kilometraje']);

			if(isset($distance) && $distance > 0) {
				// Obtenemos el evento como tal, que es a su vez un POST
				$event = get_event_info_custom($eventid);
				if(isset($event)) {
					// Obtenemos el nombre correspondiente a los books del plugin
					$table_booking = $table_prefix.DP_PRO_EVENT_CALENDAR_TABLE_BOOKING;

					// Buscamos todos los books que estén registrados en el evento
					$querystr = "SELECT * FROM $table_booking WHERE id_event = %d ORDER BY id";
					...
					foreach($bookings_obj as $booking) {
						...
						if(is_numeric($id_user) && $id_user > 0) {
							// Procesamos la información, para que incremente el kilometraje del usuario
							proccess_on_action_booking($eventid, $distance, $id_user, true);
					...
					// Actualizamos la bandera que nos va a indicar que el evento esta cerrado
					update_post_meta($eventid, 'pec_custom_aux_estatus', 1);
					// Actualizamos el valor del kilometraje.
					update_post_meta($eventid, 'pec_custom_kilometraje', $distance);

					// Por último actualizamos el estatus a todos los books del evento
					$querystr = "UPDATE $table_booking SET status = 'asistio' WHERE id_event = %d";
					$wpdb->query($wpdb->prepare($querystr, $eventid));

					$response['result'] = true;
				}
			}
			else {
				$response['message'] = 'El kilometraje no existe.';
			}
		}
	}
...
add_action( 'wp_ajax_nopriv_changeAllStatusBookins', 'dpProEventCalendar_changeAllStatusBookins' );
add_action( 'wp_ajax_changeAllStatusBookins', 'dpProEventCalendar_changeAllStatusBookins' );

function dpProEventCalendar_changeAllStatusBookins() {
	header("HTTP/1.1 200 OK");
	global $wpdb, $table_prefix;
	// Inicializamos el resultado en falso
	$response = array("result" => false);
	...
	if(is_user_logged_in()) {
		...
		// Comprobamos que nos hayan mandando el id del evento
		if(is_numeric($_POST['eventid'])) {
			...
			if(isset($event)) {
				...
				foreach($bookings_obj as $booking) {
					...
					if(is_numeric($id_user) && $id_user > 0) {
						if(empty($status)) {
							proccess_on_action_booking($eventid, $distance, $id_user, false);
						}
						else {
							// Procesamos la información, para que incremetne el kilometraje del usuario
							proccess_on_action_booking($eventid, $distance, $id_user, true);
						}
				...
				// Por último actualizamos el estatus a todos los books del evento
				$querystr = "UPDATE ".$table_booking." SET status = '".$status."' WHERE id_event = %d";
				$wpdb->query($wpdb->prepare($querystr, $eventid));
				$response['result'] = true;
			}
		}
...
function dpProEventCalendar_removeBooking() {
	...
	do_action('pec_action_remove_book_event', $booking_id);
	on_remove_book_event_change($booking_id);
...
function dpProEventCalendar_bookEventAdmin() {
	...
	if (isset($_POST['fromedit'])) {
		$fromedit = $_POST['fromedit'] == 'true' ? true : false;
	}
...
function dpProEventCalendar_getMoreBookings($limit = 30, $offset = '', $eventid = '', $event_date = '', $fromedit = false) {
	...
	if($event_date != "") {
		$querystr .= " AND event_date = '".$event_date."'";
	}
	$querystr .= " ORDER BY id DESC LIMIT ".$offset.", ".$limit;
	$bookings_obj = $wpdb->get_results($querystr, OBJECT);
	...
	<tr>
				...
				<?php
					...
					foreach($extra_fields as $key=>$value) {
						...
						if($value != "" && is_numeric($field_index)) {
							...
							$html .= '<div class="pec_event_page_custom_fields"><strong>'.$dpProEventCalendar['booking_custom_fields']['name'][$field_index].': </strong>'.$value;
							$html .= '</div>';
						}
					}
					echo $html;
		...
		<?php } ?>
		<td><input type="button" value="Eliminar" name="delete_booking" <?php if(!$fromedit): ?> class="button button-primary" <?php else: ?> class="btn btn-primary" <?php endif; ?> onclick="if(confirm('¿Estas seguro de que quieres eliminar este participante?')) { pec_removeBooking(<?php echo $booking->id?>, this); }" /></td>
	</tr>
	<?php
	}
	...
	$page = trim(preg_replace('/(\v|\s)+/', ' ', $page));
