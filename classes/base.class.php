...
class DpProEventCalendar {
		...
		)
	{
		...
		$this->translation = array(
			...
			'TXT_ASISTIO'				=> __('Asistio','dpProEventCalendar'),
			'TXT_INSCRITO'				=> __('Inscrito','dpProEventCalendar'),
		...
		switch($get) {
			...
			case 'date':
				...
				if(get_post_meta($post_id, 'pec_enable_booking', true) || $this->calendar_obj->booking_enable) {
					if($this->calendar_obj->booking_display_attendees) {
						$bookings = $this->getAllBookings($post_id);
						// $attendees_counter = $this->getEventBookings(true, date('Y-m-d', strtotime($event_data->date)), $post_id);
						$attendees_counter = count($bookings);

						$return .= '<div class="dp_pec_clear"></div>';
						$return .= '<div class="pec_event_page_attendees"><i class="fa fa-user"></i>';
						$return .= '<p>'.$attendees_counter.' '.($attendees_counter == 1 ? $this->translation['TXT_ATTENDEE'] : $this->translation['TXT_ATTENDEES']).'</p>';
						$return .= '</div>';

						$return .= '<div class="dp_pec_clear"></div>';

						// Creamos el nuevo elemento para contener la lista de los asistentes
						$return .= '<div class="pec_event_page_attendees">';
						$return .= '<div class="attendees_list"><ul>';

						foreach($bookings as $single) {
							$return .= '<li><i class="fa fa-check"></i><span>'. $single->name .'</span></li>';
						}
			...
			case 'actions':
				...
				if(is_user_logged_in()) {
					...
					if($current_user->ID == get_post_field( 'post_author', $post_id)) {
	...
	function getBookingForm($post_id, $date = '') {
		...
		if(!$hide_buttons) {
			...
			$return .= '<div class="pec_booking_quantity" style="display: none;">';
			...
			if($this->calendar_obj->booking_comment && !$hide_buttons && false) {
				$return .= '
	...
	function getBookingButton($post_id, $date = '', $clear = true, $show_attendees = true) {
		...
		if(
			...
			$str_disable_attr = '';
			$blnAuxStatus = get_post_meta($post_id, 'pec_custom_aux_estatus', true);
			$return .= '<strong>'.$blnAuxStatus.'</strong>';
			if(isset($blnAuxStatus)) {
				if($blnAuxStatus) {
					$str_disable_attr = ' disabled="true" ';
				}
			}
			...
			if($this->calendar_obj->booking_display_attendees && $show_attendees) {
				...
				$return .= "</div>";
				if($clear)	$return .= '<div class="dp_pec_clear"></div>';
			}
				...
				foreach($order_events as $event) {
					...
					if(is_array($dpProEventCalendar['custom_fields_counter'])) {
						...
						foreach($dpProEventCalendar['custom_fields_counter'] as $key) {
							...
							$field_value = get_post_meta($event->id, 'pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter], true);
							$aux_id = $dpProEventCalendar['custom_fields']['id'][$counter];
							if($field_value != "" && $aux_id !== "aux_estatus") {
								$html .= '<li class="pec_event_page_custom_fields pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter].'"><strong>'.$dpProEventCalendar['custom_fields']['name'][$counter].'</strong>';
								if($dpProEventCalendar['custom_fields']['type'][$counter] == 'checkbox') {
									$html .= '<i class="fa fa-check"></i>';
								} else {
						...
						$html .= '
							<form name="dp_pec_event_form" class="add_new_event_form" enctype="multipart/form-data" method="post">
								<div class="pec-add-body">
									<div class="">
										<div class="dp_pec_row dp_pc_row_required">
										...
										<div class="dp_pec_row dp_pec_cal_new_sub">
														...
														<div class="dp_pec_row">
															<div class="dp_pec_col12">
																...
																foreach ($categories as $category) {
																		...
																		$html .= '<input type="checkbox" name="category-'.$category->term_id.'" class="checkbox" value="'.$category->term_id.'" '.($bln_first ? ' checked="" ' : '').' />';
																		$html .= $category->cat_name;
																		$html .= '</div>';
																		if($bln_first) $bln_first = false;
																	}
																$html .= '
																<div class="dp_pec_clear"></div>
															</div>
											...
											<div class="dp_pec_col6">
												...
												if($this->calendar_obj->form_show_link) {
													$html .= '<div class="dp_pec_row"><input type="url" value="" placeholder="'.$this->translation['TXT_EVENT_LINK'].'" id="" name="link" /></div>';
												}

												if($this->calendar_obj->form_show_phone) {
													$html .= '<div class="dp_pec_row"><input type="text" value="" placeholder="'.$this->translation['TXT_EVENT_PHONE'].'" id="" name="phone" /></div>';
												}
												...
												if(is_array($dpProEventCalendar['custom_fields_counter'])) {
													...
													foreach($dpProEventCalendar['custom_fields_counter'] as $key) {
														...
														if($dpProEventCalendar['custom_fields']['type'][$counter] == "checkbox") {
															...
															if($aux_id == 'aux_estatus') {
																$html .= '<div style="display: none;">';
															}

															$html .= '
															<div class="dp_pec_wrap_checkbox" data-here="test2">
															...
															$html .= '
															<div class="dp_pec_row '.(!$dpProEventCalendar['custom_fields']['optional'][$counter] ? 'dp_pc_row_required' : '').'">
											...
											<div class="dp_pec_row">
												<p><strong>NOTA:</strong> Los campos marcados con <span class="asterisk-content"><span class="asterisk-aux">*</span></span> son obligatorios.</p>
											</div>
											<div class="dp_pec_clear"></div>
										</div>
									</div>
						...
						<div class="dp_pec_row">
							...
							<div class="dp_pec_col6">
								...
								if($this->calendar_obj->form_show_image) {
									...
									$html .= '
									<span class="dp_pec_form_desc">'.$this->translation['TXT_EVENT_IMAGE'].'</span>
									<div class="dp_pec_add_image_wrap" data-here="2">
										<label for="event_image_'.$this->nonce.'_'.$rand_image.'"><span class="dp_pec_add_image"></span></label><input type="text" class="dp_pec_new_event_text" value="" readonly="readonly" id="event_image_lbl" name="" />
	...
	function getEditRemoveButtons($event) {
		...
		if(is_user_logged_in() && (($this->calendar_obj->allow_user_edit_event || $this->calendar_obj->allow_user_remove_event) && $this->opts['allow_user_edit_remove'])) {
			...
			if($current_user->ID == get_post_field( 'post_author', $event->id)) {
	...
	function getAddForm($edit = false) {
		global $dpProEventCalendar, $table_prefix, $dp_pec_payments, $wpdb;
		...
		$str_disable_attr = '';
		$blnAuxStatus = get_post_meta($edit, 'pec_custom_aux_estatus', true);
		if(isset($blnAuxStatus)) {
			if($blnAuxStatus) {
				$str_disable_attr = ' disabled="true" ';
			}
		...
		$html .= '
			<div class="dp_pec_row dp_pc_row_required">
				<input type="text" class="dp_pec_new_event_text dp_pec_form_title pec_required" value="'.$title.'" placeholder="'.$this->translation['TXT_EVENT_TITLE'].'" name="title" '.$str_disable_attr.' />
			</div>
			';
			if($this->calendar_obj->form_show_description) {
				...
				if($this->calendar_obj->form_text_editor) {
					...
					$html .= '
					<div class="dp_pec_row">
						<textarea placeholder="'.$this->translation['TXT_EVENT_DESCRIPTION'].'" class="dp_pec_new_event_text" id="" name="description" cols="50" rows="5" '.$str_disable_attr.'>'.$description.'</textarea>
					</div>';
				}
			...
			<div class="dp_pec_row">
				<div class="dp_pec_col6">
							...
							<div class="dp_pec_row">
								<div class="dp_pec_col12">
									...
									foreach ($categories as $category) {
										...
										$html .= '<div class="pec_checkbox_list">';
										$html .= '<input type="checkbox" name="category-'.$category->term_id.'" class="checkbox" value="'.$category->term_id.'" '.(in_array($category->term_id, $post_category_ids) ? 'checked="checked"' : '').' onchange="Woffice.onEventCategoryChangeAux(this, event);" '.$str_disable_attr.' />';
										$html .= $category->cat_name;
										$html .= '</div>';
										}
									$html .= '
					...
					<div class="dp_pec_row">
						...
						$html .= '
							...
							if(empty($str_disable_attr)) {
								$html .= '<button type="button" class="dp_pec_clear_end_date">
										<img src="'.dpProEventCalendar_plugin_url( 'images/admin/clear.png' ).'" alt="Clear" title="Clear">
									</button>';
							}
						}
					...
					if($this->calendar_obj->form_show_extra_dates) {
						$html .= '<div class="dp_pec_row">';
						$html .= '<div class="dp_pec_col12">';
						$html .= '<input type="text" value="" placeholder="'.$this->translation['TXT_EXTRA_DATES'].'" id="" class="dp_pec_extra_dates" readonly="readonly" style="max-width: 300px;" name="extra_dates" '.$str_disable_attr.' />';
						$html .= '</div>';
						$html .= '<div class="dp_pec_clear"></div>
						</div>';
					}
					...
					if($this->calendar_obj->form_show_start_time) {
						...
						<div class="dp_pec_col6">
							<span class="dp_pec_form_desc">'.$this->translation['TXT_EVENT_START_TIME'].'</span>
							<div class="dp_pec_clear"></div>
							<select autocomplete="off" class="dp_pec_new_event_time" name="time_hours" id="" style="width:'.(dpProEventCalendar_is_ampm() ? '70' : '50').'px;" '.$str_disable_attr.'>';
								for($i = 0; $i <= 23; $i++) {
									$hour = str_pad($i, 2, "0", STR_PAD_LEFT);
									if(dpProEventCalendar_is_ampm()) {
										$hour = ($hour > 12 ? $hour - 12 : ($hour == '00' ? '12' : $hour)) . ' ' . date('A', mktime($hour, 0));
							...
							<select autocomplete="off" class="dp_pec_new_event_time" name="time_minutes" id="pec_time_minutes" style="width:50px;" '.$str_disable_attr.'>';
								for($i = 0; $i <= 59; $i += 5) {
									$html .= '
					...
					if($this->calendar_obj->form_show_end_time) {
						...
						<div class="dp_pec_col6">
							<span class="dp_pec_form_desc">'.$this->translation['TXT_EVENT_END_TIME'].'</span>
							<div class="dp_pec_clear"></div>
							<select autocomplete="off" class="dp_pec_new_event_time" name="end_time_hh" id="" style="width:'.(dpProEventCalendar_is_ampm() ? '70' : '50').'px;" '.$str_disable_attr.'>
								<option value="">--</option>';
								for($i = 0; $i <= 23; $i++) {
									$hour = str_pad($i, 2, "0", STR_PAD_LEFT);
									if(dpProEventCalendar_is_ampm()) {
							...
							<select autocomplete="off" class="dp_pec_new_event_time" name="end_time_mm" id="" style="width:50px;" '.$str_disable_attr.'>
								<option value="">--</option>';
								for($i = 0; $i <= 59; $i += 5) {
									$html .= '
					...
					if($this->calendar_obj->form_show_hide_time) {
						...
						<div class="dp_pec_col6">
							<span class="dp_pec_form_desc">'.$this->translation['TXT_EVENT_HIDE_TIME'].'</span>
							<select autocomplete="off" name="hide_time" '.$str_disable_attr.'>
						...
						';
							...
							if($this->calendar_obj->form_show_all_day) {
								...
								$html .= '
								<input type="checkbox" class="checkbox" name="all_day" id="" value="1" '.($all_day ? 'checked="checked"' : '').' '.$str_disable_attr.' />
								<span class="dp_pec_form_desc dp_pec_form_desc_left">'.$this->translation['TXT_EVENT_ALL_DAY'].'</span>';
					...
					if($this->calendar_obj->form_show_frequency) {
						...
						<div class="dp_pec_col6">
							<span class="dp_pec_form_desc">'.$this->translation['TXT_EVENT_FREQUENCY'].'</span>
							<select autocomplete="off" name="recurring_frecuency" id="pec_recurring_frecuency" class="pec_recurring_frequency" '.$str_disable_attr.'>
								<option value="0">'.$this->translation['TXT_NONE'].'</option>
						...
						';
							...
							<div class="pec_weekly_frequency" '.($recurring_frecuency != 2 ? 'style="display:none;"' : '').'>
								...
								'. $this->translation['TXT_REPEAT_EVERY'].' <input type="number" min="1" max="99" style="width:60px;padding: 5px 10px;margin-bottom: 10px !important;" class="dp_pec_new_event_text" maxlength="2" name="pec_weekly_every" value="'.$pec_weekly_every.'" '.$str_disable_attr.' /> '. $this->translation['TXT_WEEKS_ON'].'

								<div class="dp_pec_clear"></div>

								<div class="pec_checkbox_list">
									<input type="checkbox" class="checkbox" value="1" '.(in_array(1, $pec_weekly_day) ? 'checked="checked"' : '').' name="pec_weekly_day[]" '.$str_disable_attr.' /> &nbsp; '. $this->translation['TXT_MON'] . '
								</div>
								<div class="pec_checkbox_list">
									<input type="checkbox" class="checkbox" value="2" '.(in_array(2, $pec_weekly_day) ? 'checked="checked"' : '').' name="pec_weekly_day[]" '.$str_disable_attr.' /> &nbsp; '. $this->translation['TXT_TUE'] . '
								</div>
								<div class="pec_checkbox_list">
									<input type="checkbox" class="checkbox" value="3" '.(in_array(3, $pec_weekly_day) ? 'checked="checked"' : '').' name="pec_weekly_day[]" '.$str_disable_attr.' /> &nbsp; '. $this->translation['TXT_WED'] . '
								</div>
								<div class="pec_checkbox_list">
									<input type="checkbox" class="checkbox" value="4" '.(in_array(4, $pec_weekly_day) ? 'checked="checked"' : '').' name="pec_weekly_day[]" '.$str_disable_attr.' /> &nbsp; '. $this->translation['TXT_THU'] . '
								</div>
								<div class="pec_checkbox_list">
									<input type="checkbox" class="checkbox" value="5" '.(in_array(5, $pec_weekly_day) ? 'checked="checked"' : '').' name="pec_weekly_day[]" '.$str_disable_attr.' /> &nbsp; '. $this->translation['TXT_FRI'] . '
								</div>
								<div class="pec_checkbox_list">
									<input type="checkbox" class="checkbox" value="6" '.(in_array(6, $pec_weekly_day) ? 'checked="checked"' : '').' name="pec_weekly_day[]" '.$str_disable_attr.' /> &nbsp; '. $this->translation['TXT_SAT'] . '
								</div>
								<div class="pec_checkbox_list">
									<input type="checkbox" class="checkbox" value="7" '.(in_array(7, $pec_weekly_day) ? 'checked="checked"' : '').' name="pec_weekly_day[]" '.$str_disable_attr.' /> &nbsp; '. $this->translation['TXT_SUN'] . '
								</div>
							...
							<div class="pec_monthly_frequency" '.($recurring_frecuency != 3 ? 'style="display:none;"' : '').'>
								...
								'. $this->translation['TXT_REPEAT_EVERY'].' <input type="number" min="1" max="99" style="width:60px;padding: 5px 10px;margin-bottom: 10px !important;" class="dp_pec_new_event_text" maxlength="2" name="pec_monthly_every" value="'.$pec_monthly_every.'" '.$str_disable_attr.' /> ' . $this->translation['TXT_MONTHS_ON'] . '

								<div class="dp_pec_clear"></div>

								<select autocomplete="off" name="pec_monthly_position" id="pec_monthly_position" style="width:90px;" '.$str_disable_attr.'>
									<option value=""> ' . $this->translation['TXT_RECURRING_OPTION'] . '</option>
								...
								<select autocomplete="off" name="pec_monthly_day" id="pec_monthly_day" style="width:150px;" '.$str_disable_attr.'>
								<option value=""> ' . $this->translation['TXT_RECURRING_OPTION'] . '</option>
					...
					if($this->calendar_obj->form_show_booking_enable ) {
						$html .= '
						<div class="dp_pec_col6">
							<input type="checkbox" '.($booking_enable ? 'checked="checked"' : '').' class="checkbox" name="booking_enable" id="" value="1" '.$str_disable_attr.' />
							<span class="dp_pec_form_desc dp_pec_form_desc_left">'.$this->translation['TXT_ALLOW_BOOKINGS'].'</span>
						</div>';
					}
					...
					if($this->calendar_obj->form_show_booking_price && is_plugin_active( 'dp-pec-payments/dp-pec-payments.php' ) ) {
						$html .= '
						<div class="dp_pec_col6">
							<input type="number" min="0" value="'.$price.'" class="dp_pec_new_event_text" style="width: 120px;" placeholder="'.$this->translation['TXT_PRICE'].'" id="" name="price" '.$str_disable_attr.' /> <span class="dp_pec_form_desc dp_pec_form_desc_left">'.$dp_pec_payments['currency'].'</span>
						</div>';
					}
					...
					if($this->calendar_obj->form_show_booking_block_hours ) {
						$html .= '
						<div class="dp_pec_col6">
							<input type="number" min="0" value="'.$block_hours.'" class="dp_pec_new_event_text" style="width: 140px;" placeholder="'.$this->translation['TXT_BOOKING_BLOCK_HOURS'].'" id="" name="block_hours" '.$str_disable_attr.' />
						</div>';
					}
					...
					if($this->calendar_obj->form_show_booking_limit ) {
						$html .= '
						<div class="dp_pec_col6">
							<input type="number" min="0" value="'.$limit.'" class="dp_pec_new_event_text" style="width: 140px;" placeholder="'.$this->translation['TXT_BOOKING_LIMIT'].'" id="" name="limit" '.$str_disable_attr.' />
						</div>';
					}
				...
				<div class="dp_pec_col6">
					...
					if($this->calendar_obj->form_show_image || false) {
						...
						<span class="dp_pec_form_desc">'.$this->translation['TXT_EVENT_IMAGE'].'</span>
						<div class="dp_pec_add_image_wrap">
							<label for="event_image_'.$this->nonce.'_'.$rand_image.(is_numeric($edit) ? '_pecremoveedit' : '').'"><span class="dp_pec_add_image"></span></label><input type="text" class="dp_pec_new_event_text" value="" readonly="readonly" id="event_image_lbl" name="" '.$str_disable_attr.' />
						</div><input type="file" name="event_image" id="event_image_'.$this->nonce.'_'.$rand_image.(is_numeric($edit) ? '_pecremoveedit' : '').'" class="event_image" style="visibility:hidden; position: absolute;" '.$str_disable_attr.' />
						</div>';
					}
					...
					if($this->calendar_obj->form_show_color) {
						...
						<div class="dp_pec_row">
							<span class="dp_pec_form_desc">'.$this->translation['TXT_SELECT_COLOR'].'</span>
							<select autocomplete="off" name="color" class="pec_color_form" '.$str_disable_attr.'>
								<option value="">'.$this->translation['TXT_NONE'].'</option>';
							$counter = 0;
							$querystr = "SELECT *
								FROM ". $this->table_special_dates ."
					...
					if($this->calendar_obj->form_show_timezone) {
						...
						<div class="dp_pec_row">
							<span class="dp_pec_form_desc">'.$this->translation['TXT_SELECT_TIMEZONE'].'</span>
							<select autocomplete="off" name="timezone" class="pec_timezone_form" '.$str_disable_attr.'>';
								$current_offset = get_option('gmt_offset');
								$tzstring = get_option('timezone_string');
								if ( empty($tzstring) ) { // Create a UTC+- zone if no timezone string exists
									$check_zone_info = false;
					...
					if($this->calendar_obj->form_show_location) {
						...
						<div class="dp_pec_row">
						<span class="dp_pec_form_desc">'.$this->translation['TXT_EVENT_LOCATION'].'</span>
						<select autocomplete="off" name="location" class="pec_location_form" '.$str_disable_attr.'>
							<option value="">'.$this->translation['TXT_NONE'].'</option>
							 ';
						...
						if($this->calendar_obj->form_show_location_options) {
							$html .= '<div class="pec_location_options" style="display:none;">
								<input type="text" value="" placeholder="'.$this->translation['TXT_EVENT_LOCATION_NAME'].'" id="" name="location_name" '.$str_disable_attr.' />';

								$html .= '<input type="text" value="" placeholder="'.$this->translation['TXT_EVENT_ADDRESS'].'" id="" name="location_address" '.$str_disable_attr.' />';
							$html .= '</div>';
						}
					}

					if($this->calendar_obj->form_show_link) {
						$html .= '<div class="dp_pec_row"><input type="url" class="dp_pec_new_event_text" value="'.$link.'" placeholder="'.$this->translation['TXT_EVENT_LINK'].'" id="" name="link" '.$str_disable_attr.' /></div>';
					}

					if($this->calendar_obj->form_show_phone) {
						$html .= '<div class="dp_pec_row"><input type="text" class="dp_pec_new_event_text" value="'.$phone.'" placeholder="'.$this->translation['TXT_EVENT_PHONE'].'" id="" name="phone" '.$str_disable_attr.' /></div>';
					}
					...
					if(is_array($dpProEventCalendar['custom_fields_counter'])) {
						...
						foreach($dpProEventCalendar['custom_fields_counter'] as $key) {
							...
							if($dpProEventCalendar['custom_fields']['type'][$counter] == "checkbox") {
								...
								<input type="checkbox" class="checkbox '.(!$dpProEventCalendar['custom_fields']['optional'][$counter] ? 'pec_required' : '').'" value="1" id="pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter].'" '.(is_numeric($edit) && get_post_meta($edit, 'pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter], true) ? 'checked="checked"' : '').' name="pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter].'" '.$str_disable_attr.' /> '.$dpProEventCalendar['custom_fields']['placeholder'][$counter].'
								</div>';
				...
				$html .= '
				<div id="dpProEventCalendar_booking_meta">
					<div class="dp_pec_row inside">
					<div class="dp_pec_col6">
						<p><strong>Lista de Participantes</strong></p>
					</div>
					';
				$html .= '<div class="dp_pec_col6">';
					if(current_user_can('edit_others_posts') && empty($str_disable_attr)) {
						$html .= '<a class="btn btn-primary" style="float: right;"  href="javascript:void(0);" id="pec_new_booking_show">Agregar participante</a>';
					}
				$html .= '</div>';

				// $html .= '<div class="dp_pec_row">';
				// $html .= '
				// 	<select id="all_pec_booking_status" style="padding: 8px;" '.$str_disable_attr.'>
				// 		<option value="">Inscrito</option>
				// 		<option value="asistio">Asistio</option>
				// 	</select>';

				// if(empty($str_disable_attr)) {
				// 	$html .= '<a class="btn btn-primary" id="all_pec_booking_status_apply" style="margin-left: 10px;"  onclick="Woffice.pec_changeAllStatusBookins('.$edit.', event);">Aplicar a todos</a>';
				// }
				...
				// $html .= '</div>';
					...
					if(current_user_can('edit_others_posts')) {
						...
						// Buscamos todos los books que estén registrados en el evento
						$querystr = "SELECT * FROM $table_name_booking WHERE id_event = %d ORDER BY id_user";
						$bookings_obj = $wpdb->get_results(
							$wpdb->prepare(
								$querystr,
								$edit
							), OBJECT
						...
						foreach($bookings_obj as $single) {
							if(is_numeric($single->id_user) && $single->id_user > 0) {
								if(!in_array($single->id_user, $bookings_users)) array_push($bookings_users, $single->id_user);
							}
						}

						$html .= '<div id="pec_new_booking" class="dp_pec_row" style="display: none;">
							<hr />
						...
						if(empty($str_disable_attr)) {
							$html .= '<hr />
								<a class="btn btn-primary" href="javascript:void(0);" data-dppec-eventid="'.$edit.'" id="pec_new_booking_submit">Agregar</a>
								<a class="btn" href="javascript:void(0);" id="pec_new_booking_cancel">Cancelar</a>';
						}

						$html .= '</div>'; // End #pec_new_booking

						$html .= '<table class="table" cellpadding="0" cellspacing="0" id="pec-sort-table">
							<thead>
						...
						foreach($bookings_obj as $booking) {
							...
							$html .= '<td data-status="'.$booking->status.'">'.$userdata->display_name.'<br>';
							if($userdata->user_email != "") {
								$html .= '<span class="dashicons dashicons-email-alt"></span><input type="text" readonly="readonly" name="pec_booking_email['.$booking->id.']" class="pec_booking_text" value="'.$userdata->user_email.'" /><br>';
							}

							if($booking->phone != "") {
								$html .= '<span class="dashicons dashicons-phone"></span><input type="text" readonly="readonly" name="pec_booking_phone['.$booking->id.']"  class="pec_booking_text" value="'.$booking->phone.'" />';
							}
							$html .= '</td>';

							$html .= '<td>'.date_i18n(get_option('date_format').' '.get_option('time_format'), strtotime($booking->booking_date)).'</td>';
							// $html .= '<td>
							// 	<select name="pec_booking_status['.$booking->id.']" '.$str_disable_attr.'>
							// 		<option value=""  '.($booking->status == "" ? 'selected="selected"' : '').'>Inscrito</option>
							// 		<option value="asistio" '.($booking->status == "asistio" ? 'selected="selected"' : '').'>Asistio</option>
							// 	</select>';

							// 	if(empty($str_disable_attr)) {
							// 		$html .= '<a class="btn btn-primary" style="margin-left: 5px;" id="cur_pec_booking_status_update" onclick="Woffice.pec_changeCurrentBooking('.$booking->id.', event);"><span style="line-height: 26px; font-size: 18px;" class="dashicons dashicons-image-rotate"></span></a>';
							// 	}

							// $html .= '</td>';
							$html .= '<td>';
								if(empty($str_disable_attr)) {
									$html .= '<input type="button" value="Eliminar" name="delete_booking" class="btn btn-primary" onclick="if(confirm(\'¿Estas seguro de que quieres eliminar este participante?\')) { Woffice.pec_removeBooking('.$booking->id.', this); }" />';
								}
							$html .= '</td>';
			...
			$html .= '</div>'; // End #dpProEventCalendar_booking_meta
			if(is_numeric($id_calendar)) {
				$html .= '</div>';
			}
			...
			if(empty($str_disable_attr)) {
				$html .= '<button style="margin: 0 1.2em 0 0!important" class="dp_pec_submit_event" data-lang-sending="'.$this->translation['TXT_SENDING'].'">Guardar Cambios</button>
				<button style="clear: inherit;" type="button" class="dp_pec_cancel_booking dp_pec_end_event">Cerrar Evento</button>';
			}
	...
	function getAllBookings($event_id) {
		global $wpdb, $dpProEventCalendar, $table_prefix;

		$querystr = "SELECT * FROM ".$this->table_booking." WHERE id_event = %d ORDER BY id";
		$bookings_obj = $wpdb->get_results(
			$wpdb->prepare(
				$querystr,
				$event_id
		...
		foreach($bookings_obj as $booking) {
			...
			if(is_numeric($id_user) && $id_user > 0) {
				...
				if(isset($userdata)) {
					$temp = new stdClass();
					$temp->name = $userdata->display_name;
					array_push($bookings, $temp);
				}
			}
		}
